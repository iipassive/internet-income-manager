...
# Usage:
#   sudo ./install.sh
#   sudo ./install.sh http://other-control.example.com:18881   # override URL
##############################################################################
set -e

CONTROL_URL="${1:-http://mainsite.vinaproxy.net:18881}"
LOG=$(mktemp /tmp/iim-install.XXXXXX.log 2>/dev/null || mktemp /var/tmp/iim-install.XXXXXX.log 2>/dev/null || echo /dev/null)
